...
def top5teams():
    ...
    for team in home.find_all("div", {"class": ["col-box rank"], }):
        ...
        teams.append(team)
    return teams
def get_team_rank_by_date(data):
    urls=[]
    ...
    page = get_parsed_page(final_urls[0])
    teamlists = []
    for x in range(len(final_urls)):
        if final_urls[x] != final_urls[x-1]:
            page = get_parsed_page(final_urls[x])
        teams = page.find("div", {"class": "ranking"})
        stroke={}
        stroke['match_id']=data['match_id'][x]
        for team in teams.find_all("div", {"class": "ranked-team standard-box"}):
            
            if str(data['team1'][x]).strip() == team.find('div', {"class": "ranking-header"}).select('.name')[0].text.strip():
                stroke['name_team_1'] = team.find('div', {"class": "ranking-header"}).select('.name')[0].text.strip()

                stroke['rank_team_1'] = converters.to_int(team.select('.position')[0].text.strip(), regexp=True)
                stroke['rank-points_team_1'] = converters.to_int(team.find('span', {'class': 'points'}).text, regexp=True)
            
                
                
            elif str(data['team2'][x]).strip() == team.find('div', {"class": "ranking-header"}).select('.name')[0].text.strip():
                stroke['name_team_2'] = team.find('div', {"class": "ranking-header"}).select('.name')[0].text.strip()

                stroke['rank_team_2'] = converters.to_int(team.select('.position')[0].text.strip(), regexp=True)
                stroke['rank-points_team_2'] = converters.to_int(team.find('span', {'class': 'points'}).text, regexp=True)
        
        
            
        teamlists.append(stroke)
...
def get_results():
    ...
    for i in range(100, 1000, 100):
        ...
        results = get_parsed_page("https://www.hltv.org/results?offset=" + str(i) +"&stars=1")
        ...
        for res in resultDiv:
            ...
            for c in res.find("a", {"class": "a-reset"}).get("href"):
                if c.isdigit():
                    num += c
                        
            resultObj['match_id'] = converters.to_int(res.find("a", {"class": "a-reset"}).get("href").split("/")[-2])
            ...
            if (res.find_all("td", {"class": "team-cell"})):
                ...
                resultObj['team1_id'] = _findTeamId(res.find_all("td", {"class": "team-cell"})[0].text.lstrip().rstrip())
                resultObj['team2'] = res.find_all("td", {"class": "team-cell"})[1].text.lstrip().rstrip()
                resultObj['team2_id'] = _findTeamId(res.find_all("td", {"class": "team-cell"})[1].text.lstrip().rstrip())
                ...
                resultObj['team1_id'] = None
                resultObj['team1score'] = None
                resultObj['team2'] = None
                resultObj['team2_id'] = None
...
def get_match_inf(matches_id):
    result_list = []
    for match in matches_id:
        url = ("https://hltv.org/matches/" + str(match) + "/page")
        
        match_page = get_parsed_page(url)
        resultObj = {}   
        n=0
        maps = match_page.find_all("div", {"class": "mapholder"})
        resultObj['match_id'] = match
        for ma in maps:
            n+=1            
            if (ma.find("div", {"class": "results played"})):
                resultObj['team1score_ct_on_map_'+str(n)] = converters.to_int(ma.find("div", {"class": "results-center-half-score"}).find_all("span")[1].text)
                resultObj['team1score_t_on_map_'+str(n)] = converters.to_int(ma.find("div", {"class": "results-center-half-score"}).find_all("span")[5].text)
                resultObj['team2score_ct_on_map_'+str(n)] = converters.to_int(ma.find("div", {"class": "results-center-half-score"}).find_all("span")[7].text)
                resultObj['team2score_t_on_map_'+str(n)] = converters.to_int(ma.find("div", {"class": "results-center-half-score"}).find_all("span")[3].text)
                resultObj['name_of_map_'+str(n)] = ma.find("div", {"class": "mapname"}).text
            
        result_list.append(resultObj)
    return result_list
def get_maps_inf_from_match(matches_id):
    result_list = []
    for match in matches_id:
        url = ("https://hltv.org/matches/" + str(match) + "/page")
        ...
        resultObj['match_id'] = match
        n=0
        maps = match_page.find_all("div", {"class": "results-center-stats"})
        for ma in maps: 
            n+=1
            
            if(ma.find("a", {"class": "results-stats"}) is not None):
               
                map_url= ma.find("a", {"class": "results-stats"}).get("href")
                resultObj['url_of_map_'+str(n)] = ("https://hltv.org" + map_url)
                map_parsed=get_parsed_page("https://hltv.org" + map_url)
                rows_data=map_parsed.find_all("div", {"class": "match-info-row"})
                
                if(str(rows_data[0].find("div", {"class": "right"}).find_all("span")[2].get("class")) == "['t-color']"): 
                    resultObj['team1score_t_on_map_'+str(n)] = converters.to_int(rows_data[0].find("div", {"class": "right"}).find_all("span", {"class":"t-color"})[0].text)
                    resultObj['team1score_ct_on_map_'+str(n)] = converters.to_int(rows_data[0].find("div", {"class": "right"}).find_all("span", {"class":"ct-color"})[1].text)
                    resultObj['team2score_t_on_map_'+str(n)] = converters.to_int(rows_data[0].find("div", {"class": "right"}).find_all("span",{"class":"t-color"})[1].text)
                    resultObj['team2score_ct_on_map_'+str(n)] = converters.to_int(rows_data[0].find("div", {"class": "right"}).find_all("span",{"class":"ct-color"})[0].text)
                else:
                    resultObj['team1score_t_on_map_'+str(n)] = converters.to_int(rows_data[0].find("div", {"class": "right"}).find_all("span", {"class":"t-color"})[1].text)
                    resultObj['team1score_ct_on_map_'+str(n)] = converters.to_int(rows_data[0].find("div", {"class": "right"}).find_all("span", {"class":"ct-color"})[0].text)
                    resultObj['team2score_t_on_map_'+str(n)] = converters.to_int(rows_data[0].find("div", {"class": "right"}).find_all("span",{"class":"t-color"})[0].text)
                    resultObj['team2score_ct_on_map_'+str(n)] = converters.to_int(rows_data[0].find("div", {"class": "right"}).find_all("span",{"class":"ct-color"})[1].text)
                j=0
                for row in rows_data[1:]:
                    j+=1
                    resultObj[str(j)+'_feature_value_on_map_'+str(n)]=row.find("div",{"class": "right"}).text
                    resultObj[str(j)+'_feature_name_on_map_'+str(n)]=row.find("div",{"class": "bold"}).text
